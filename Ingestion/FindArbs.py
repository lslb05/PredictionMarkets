...
class CrossSumArbitrage:
    ...
    def get_poly_ask(self, token_id):
        """Busca o menor preço de VENDA (Ask) no Polymarket"""
        try:
    ...
    def get_kalshi_bids(self, ticker):
        """Busca o maior preço de COMPRA (Bid) na Kalshi"""
    ...
    def run(self):
        ...
        print(f"🔄 Escaneando {len(df)} mercados (Busca pelo MELHOR Spread)...")
        ...
        for _, row in df.iterrows():
            ...
            # Variáveis temporárias para comparar as estratégias
            op1 = None
            op2 = None

            # --- ESTRATÉGIA 1: Kalshi YES (Bid) + Poly NO (Ask) ---
            ...
            # --- ESTRATÉGIA 2: Kalshi NO (Bid) + Poly YES (Ask) ---
            ...
            # --- LÓGICA DE DECISÃO (QUAL É MELHOR?) ---
            chosen_op = None

            if op1 and op2:
                # Se as duas derem oportunidade, pega a de maior lucro (spread maior)
            ...
            # Se escolheu alguma, adiciona à lista final
            if chosen_op:
                all_opportunities.append(chosen_op)

        # --- EXIBIÇÃO ---
        if not all_opportunities:
            print("\n❌ Nenhuma oportunidade de Maker (Soma <= 0.98) encontrada.")
        else:
            # Ordena pelo maior spread implícito
            sorted_arbs = sorted(all_opportunities, key=lambda x: x['implied_spread'], reverse=True)
            
            print("\n" + "="*80)
            print(f"🚨 {len(sorted_arbs)} MELHORES OPORTUNIDADES ENCONTRADAS")
            print("="*80)
            
            for arb in sorted_arbs:
                # Cores condicionais (opcional, se terminal suportar)
                symbol = "🟢" if "Yes(Bid)" in arb['type'] else "🔵"
                
                print(f"{symbol} Spread: {arb['implied_spread']:.1f}¢ | Soma: ${arb['sum']:.2f}")
                print(f"   Mercado:    {arb['market']}")
                print(f"   Estratégia: {arb['type']}")
                print(f"   {arb['details']}")
                print("-" * 80)

if __name__ == "__main__":
    monitor = CrossSumArbitrage()
    monitor.run()